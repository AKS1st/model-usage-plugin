...
**价目按官方页刷新（2026-09-14）**：[DeepSeek 模型 & 价格](https://api-docs.deepseek.com/quick_start/pricing)、
[Z.AI Pricing](https://docs.z.ai/guides/overview/pricing)、[OpenAI Pricing](https://developers.openai.com/api/docs/pricing)。
本次修正了三处会**把钱算错**的问题：GPT-5.6 三档（luna/terra/sol）此前只记了官方价的一半、
GPT-6 Astra 与 GLM-5.2/5.3 漏了"缓存命中"价（读缓存被当成未命中 → 高估）；另外官方已明确
DeepSeek V4 Pro **继续提供且计费不变**，因此删掉了此前"9/14 之后改按 Flash 价计费"的规则
（留着会把 V4-Pro 的用量低估约 5 倍）。
...
### 两个开关：自定义计费 / Token Plan

每个模型展开后都有两个开关，用来控制"这份用量该怎么算钱"：

| 开关 | 关闭（默认） | 打开 |
| --- | --- | --- |
| **自定义计费** | 用**内置默认价**（含官方峰谷规则），单价框只读 | 用**你自己填的**单价与峰谷时段 |
| **Token Plan** | 按 token 计费 | 该模型由套餐覆盖：**只记录 token，不算费用** |

- 只对**支持套餐**的模型显示 Token Plan 开关（OpenAI/Anthropic/Google/GLM/Qwen/Kimi 等；
  DeepSeek 只有充值余额、没有套餐，所以不显示）。
- **订阅型 provider 自动算套餐**：当某模型的用量来自订阅（如 `openai-codex` 的 Codex 订阅），
  Token Plan 会**默认勾上**——这类用量本来就不按量计费，按 API 单价算出来的金额是假的。
  想反过来看"若走 API 会花多少"，取消勾选即可（明确的取消会一直算数）。
- **计价来源可指定**：同一个模型 id 在不同 provider 上可能单价不同（`glm-5.1` 在阿里云百炼是
  CNY 6/24/1.6、在 Z.AI 是 USD 1.4/0.26/4.4）。卡片里可按观测到的 provider 逐个设置覆盖价，
  单来源模型自动选定并标注；多个来源又没指定时会明确提示"按基础价计费"。
- **来回切换不会丢数据**：你填过的单价与峰谷规则一直保留在记录里，关掉自定义计费只是
  "暂时不用它"，勾回来就还在。切换开关时只发送开关本身，不会用内置默认价覆盖你的值。

### 峰谷定价

内置默认价里已按官方窗口配好：DeepSeek 为 UTC 周一至周五 01:00–04:00 与 06:00–10:00
（即北京时间 09:00–12:00、14:00–18:00），高峰价 = 空闲价 × 2。勾上"自定义计费"后可改：
最多两个高峰时段（`HH:MM`，支持跨零点），可指定时区与"仅周一至周五"，高峰价留空则沿用正常价。
